...
int main()
{
    ...
    // --------------------
    GLFWwindow* window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "CS 562 Project 4 (SSAO)", NULL, NULL);
    if (window == NULL)
    {
    ...
    Shader pbrShader(glswGetShader("deferredPBR.Vertex"), glswGetShader("deferredPBR.Fragment"));
    ...
    Model meshModelA(std::string(PATH + "/OpenGL/models/Lucy.obj").c_str());
   ...
   // meshModels.push_back(&meshModelB);
    ...
    float shadowScalar = 1.3;   // sigma
    float shadowContrast = 0.5; // kappa
    ...
    pbrShader.setUniformInt("shadowMap", 4);
    ...
    pbrShader.setUniformInt("shadowMethod", ShadowMethod);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        if (bilateralBlur)
        {
            ...
            gBuffer.bindInput(0); // G-Buffer world position (xyz) + depth (w)
            glActiveTexture(GL_TEXTURE3);
            gBuffer.bindInput(1); // G-Buffer world normal
